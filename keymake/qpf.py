...
import keymake.painter as painter
...
def _save(im, fp, filename):
    ...
    def _write_frame(idx, frame, last_frame):
        ...
        converted = painter.convert_requested_format(this_frame, format)
        graphic_data = painter.convert_image_bytes(converted, format)

        # Convert the raw data to RLE-encoded if requested
        raw_data = graphic_data[1]
        if use_rle:
            rle_data = painter.compress_bytes_qmk_rle(graphic_data[1])
